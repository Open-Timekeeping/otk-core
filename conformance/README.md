...
The conformance suite that verifies whether a detector adapter, simulator feed, native firmware, producer client, timing-node plugin, timebase adapter, storage backend, or third-party implementation satisfies the Open Timekeeping contracts described in [`spec`](../spec).
